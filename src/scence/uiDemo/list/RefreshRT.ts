...
*/
export default class RefreshRT extends ui.uiDemo.list.RefreshUI {
    /** 滚动条效果是否停止 */
    ...
    onListMouse(e: Laya.Event, index: number): void {
        ...
        //点击标记文本
        if (e.target.name == "flag" && e.type == Laya.Event.CLICK) this.onClickFlag(index);
        //点击删除文本
        if (e.target.name == "del" && e.type == Laya.Event.CLICK) this.onClickDel(index);
    }
