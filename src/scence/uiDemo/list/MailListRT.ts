 ...
 */
export default class MailListRT extends ui.uiDemo.list.MailListUI {
    /** 记录当前的选项 */
    private optStatus: Array<number> = [];
    constructor() { super(); }

    onEnable(): void {
        const jsonPath: string = "json/mailList.json";
        Laya.loader.create(jsonPath, Laya.Handler.create(this, () => {
            //获取模拟list数据的json文件
            let jsonData = ((Laya.loader.getRes(jsonPath)) as any).mailList;
     ...
     */
    timestampFormat(fmt: string = "YYYY-MM-DD hh:mm:ss", timestamp: number = 0): string {
        Date.prototype["Format"] = function (fmt: string) {
            ...
            //检查test(fmt)里，是否存在
            if (new RegExp("(Y+)").test(fmt)) fmt = fmt.replace(RegExp.$1, (<any>(this.getFullYear() + "")).substr(4 - RegExp.$1.length));
            for (var k in o)
                if (new RegExp("(" + k + ")").test(fmt)) fmt = fmt.replace(RegExp.$1, (RegExp.$1.length == 1) ? (o[k]) : ((<any>("00" + o[k])).substr(("" + o[k]).length)));
            return fmt;
        }

        if (timestamp == 0) return new Date()["Format"](fmt);
        else return new Date(timestamp)["Format"](fmt);
    }
