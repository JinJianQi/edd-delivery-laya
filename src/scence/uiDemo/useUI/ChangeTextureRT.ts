...
export default class ChangeTextureRT extends ui.uiDemo.useUI.ChangeTextureUI {
    ...
    changeFillTexture(): void {
        ...
        this.graphics.clear();
        let _texture: Laya.Texture = Laya.loader.getRes(this.randomImg(true, 4));
        //重绘纹理
    ...
    changeTexture(): void {
        ...
        //加载资源获得一个纹理对象
        let _texture: Laya.Texture = Laya.loader.getRes(this.randomImg(true, 8, 7));
    ...
    randomImg(isMan: boolean = false, max: number = 8, min: number = 1): string {
        ...
        if (isMan) return "role/m" + mum + ".png";
