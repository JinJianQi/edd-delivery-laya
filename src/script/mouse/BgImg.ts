export default class bgImg extends Laya.Script {
    ...
    onMouseWheel(): void {
        ...
        this.owner.on(Laya.Event.MOUSE_WHEEL, this, (e: Laya.Event) => {
            ...
            if (_offsetX > this.maxX) this._owner.x = this.maxX;
            else if (_offsetX < this.minX) this._owner.x = this.minX;
            else this._owner.x = _offsetX;

            if (_offsetY > this.maxY) this._owner.y = this.maxY;
            else if (_offsetY < this.minY) this._owner.y = this.minY;
            else this._owner.y = _offsetY;
        });
    }
    ...
    updateLimit(): void {
        //父节点，panel
        let _parent = this.owner.parent.parent as Laya.Panel;
